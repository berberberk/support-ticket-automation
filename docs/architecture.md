...
## HTTP-граница прототипа

В прототипе FastAPI обрабатывает `POST /tickets`: валидирует структуру запроса, создаёт `Ticket` и синхронно вызывает существующий `SupportPipeline` в том же процессе. `GET /health` возвращает состояние и выбранные безопасные идентификаторы адаптеров (`lexical`/`qdrant`, `deterministic`/`openrouter`), но не ключи или URL. Этот слой показывает стабильный контракт для клиента; он не реализует очередь, фонового исполнителя или асинхронную генерацию.

В целевой архитектуре путь остаётся разделённым: API → fast path → durable queue → async slow path. HTTP-интерфейс не меняет правила риска: hard-risk по-прежнему прекращает путь до генератора.
...
В целевой архитектуре операторское решение и причина эскалации также попадают в аудит; локальный PoC фиксирует сам факт эскалации и её причину.
...
### Необязательные демонстрационные адаптеры

- `QdrantRetriever`: Qdrant `:memory:` и multilingual FastEmbed по утверждённой локальной KB; ошибка инициализации/поиска явно помечается и переводит поиск на `LexicalRetriever`;
- `OpenRouterGenerator`: BYOK-вызов через HTTPX с лимитом токенов и тайм-аутом; получает только обезличенный тикет и одну статью evidence. Hard-risk, отсутствие evidence, пустой ответ, ошибка или тайм-аут не дают автоматического ответа;
- фабрика выбирает адаптеры по `.env`; значения по умолчанию остаются `lexical` и `deterministic`.
...
- операторский UI;
- полный MLOps.
- развёрнутый отказоустойчивый Qdrant, семантическая оценка groundedness, инфраструктура retry/backpressure.
