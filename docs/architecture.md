...
В прототипе FastAPI обрабатывает `POST /tickets`: валидирует структуру запроса, создаёт `Ticket` и синхронно вызывает существующий `SupportPipeline` в том же процессе. `GET /health` возвращает только состояние сервиса. Этот слой показывает стабильный контракт для клиента; он не реализует очередь, фонового исполнителя или асинхронную генерацию.
