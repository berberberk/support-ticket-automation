...
- Audit trail хранит версии компонентов, evidence, причины автоматических решений и маршрут эскалации без ненужного raw текста, чтобы решения PoC можно было проверять и воспроизводить; отдельная запись результата оператора относится к target design.
